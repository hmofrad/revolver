...
public class Spinner {
	...
	public static class ComputeMigration
		...
		public void compute(
            ...
            if(vertex.getId().get() == 4)
                    System.out.println("compute Migrations");
	...
	public static class Initializer
		...
		public void compute(
			...
			if (partition == -1) {
                partition = (short) Math.ceil(vertex.getId().get() / range); // Range partitioning
			}
		...
		public void preSuperstep() {
            ...
            numberOfVertices =  getTotalNumVertices();
            range = numberOfVertices / numberOfPartitions;
            if((numberOfVertices % range) != 0) {
                range++;
            }
		}
	}
	...
	public static class Repartitioner
		...
		public void compute(
			...
			short partition;
			short currentPartition = vertex.getValue().getCurrentPartition();
            if (numberOfPartitions + repartition <= 0) {
                throw new RuntimeException("Repartitioner called with "
						+ REPARTITION + " set to 0");
            } else {
                partition = (short) Math.ceil(vertex.getId().get() / range); // Range repartitioning
		...
		public void preSuperstep() {
            ...
            numberOfVertices =  getTotalNumVertices();
            numberOfVertices =  getTotalNumVertices();
            range = numberOfVertices / (numberOfPartitions + repartition);
            if((numberOfVertices % range) != 0) {
                range++;
            }
		}
    ...
    public static class Hash
		...
		public void compute(
            ...
            //short partition = (short) (vertex.getId().get() % numberOfPartitions);
            //vertex.getValue().setCurrentPartition(partition);
			//vertex.getValue().setNewPartition(partition);
            //if(vertex.getId().get() == 4)
            //    System.out.println(vertex.getId().get() + " " + vertex.getValue().getNewPartition() + " " + vertex.getValue().getCurrentPartition());
            // update neighbors partitions
			updateNeighborsPartitions(vertex, messages);
            int totalLabels = computeNeighborsLabels(vertex);
